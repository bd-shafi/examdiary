...
$string ='<div id="accordiontop"> ';
     ...
     if($ddata['designations'] !=''){
          ...
          if ($exdatel == $calldateformatting) {
              ...
              $co++;
               ...
               if (isset($_GET['searchingfor']) && isset($_GET['date'])) {
                if (!empty($ddata['date'])) {
                    $students = (isset($ddata['students']) && is_array($ddata['students'])) ? $ddata['students'] : [];
            
                    if (!in_array($userid, $students)) {
                        $examdateremaining .= "<span class='uniqid addtolist addtolisthide" . htmlspecialchars($ddata['uniqid']) . "' uniqid='" . htmlspecialchars($ddata['uniqid']) . "' jobid='" . htmlspecialchars($row['id']) . "'>
                            <i class='fa fa-tag taskremaining' aria-hidden='true'></i>  </span>";
                        $examdateremaining .= "<span class='response" . htmlspecialchars($ddata['uniqid']) . "'>  </span>";
                    } else {
                        $examdateremaining .= "<span class='responsedone" . htmlspecialchars($ddata['uniqid']) . "'> <i class='fa fa-star taskdone' aria-hidden='true'></i> </span>";
                    }
                }
            }
  ...
  if($co >0){
     ...
     <a class="card-link plusminustop " getclass="plusminustop'.$row['id'].'" data'.$bs.'toggle="collapse" aria-expanded="true" aria-controls="#collapseTop'.$row['id'].'"  href="#collapseTop'.$row['id'].'">
         <b> '.$i.' .   </b> '.$row['organization'].'  <span class="float-right collpseex plusminustop'.$row['id'].'">+</span>
      </a>
    </div>
             ';       
 ...
 */ 
 ...
 if($access==1){
  $uldata .="<li class='list-group-item'><a href='/edit.php?jid=".$row['id']."'>Edit</a></li>";
  }
   ...
   jQuery( '.plusminustop' ).click(function() {
      ...
      var error = 0;				
			...
			if(!error){
				...
				jQuery.ajax({
				url: 'ajaxaction.php?addtolist=true',
				type: 'POST',
...
$string .=  '<p>
	   <a target="_blank" href="/jobinput.php" class="addexambutton btn btn-success btn-sm" calldate="" addexam="yes">
					 <i class="fa fa-plus" aria-hidden="true"></i> Add more circular for you and other
					</a></p>
	   '; 
$string .=$jscript;
if($i ==0){
